...
Create a new repository in your account, e.g. `reading-room-website`.
Upload every file in this folder, keeping the same structure (`index.html` at the root, `css/`, `js/`,
`data/`, `images/` alongside it).
On GitHub: Settings → Pages → Source → Deploy from a branch → `main` / `root` → Save.
Your site will appear at `https://YOUR-USERNAME.github.io/reading-room-website/` within a minute or two.
If you want a custom domain later, add it in the same Pages settings screen and add the DNS records
GitHub gives you.
Editing content
...
The catalog page embeds TinyCat, LibraryThing's catalog/circulation system for
...
Setup:
Sign up at librarycat.org and catalog your books there.
Your public catalog lives at `librarycat.org/lib/YOUR_LIBRARY_ID`.
In `catalog.html`, replace `YOUR_LIBRARY_ID` (it appears twice — in the iframe `src` and the button link
...
Create (or find) your group on radar.squat.net and note its group ID
(the number at the end of your group's URL).
In `agenda.html`, replace `YOUR_GROUP_ID` in the iframe `src` and the link below it with that number.
Add and edit events on Radar itself — the embedded list updates automatically, nothing to change on
your own site.
If you'd rather not use an iframe, Radar also has a lightweight JavaScript widget and a WordPress plugin —
see their embedding guide for both.
Contact form
...
with no backend, sign up for a free plan at Formspree and swap the form's `action`
